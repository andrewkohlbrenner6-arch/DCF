...
# -*- coding: utf-8 -*-
"""
Created on Thu Apr 30 14:56:45 2026
...
# =============================================================================
st.sidebar.markdown("### ⚙️ Operating Assumptions")
...
# =============================================================================
st.sidebar.markdown("### 🏦 Discount Rate (WACC)")
...
# =============================================================================
st.sidebar.markdown("### ♾️ Terminal Value")
...
# Equity bridge values pulled directly from Yahoo Finance (no override).
# These feed the equity bridge calculation later: EV - Debt + Cash, divided by shares.
debt_input = round(total_debt / 1e6, 1) if total_debt else 0.0
cash_input = round(total_cash / 1e6, 1) if total_cash else 0.0
shares_input = round(shares_out / 1e6, 1) if shares_out else 100.0
...
tab_overview, tab_assumptions, tab_dcf, tab_results, tab_sensitivity = st.tabs(
    ["📊 Overview", "📋 Assumptions", "🏗️ DCF Build", "🎯 Results", "🌡️ Sensitivity"]
)
...
with tab_dcf:
    ...
    st.caption("Each row applies the formula: FCF = Revenue × EBIT margin × (1 − tax) − Reinvestment, then discounted at WACC. Hover over any column header to see the underlying calculation.")

    # Build the projection table with currency formatting and a Calculation column
    # that shows the exact arithmetic for each year, so a reviewer can trace any number.
    proj_df = pd.DataFrame({
        "Year": years,
        "Revenue ($M)": [f"${x:,.1f}" for x in revenues],
        "EBIT ($M)": [f"${x:,.1f}" for x in ebits],
        "NOPAT ($M)": [f"${x:,.1f}" for x in nopats],
        "Reinvestment ($M)": [f"${x:,.1f}" for x in reinvestments],
        "FCF ($M)": [f"${x:,.1f}" for x in fcfs],
        "Discount Factor": [f"{x:.4f}" for x in discount_factors],
        "PV of FCF ($M)": [f"${x:,.1f}" for x in pv_fcfs],
        "EBIT Calculation": [
            f"${revenues[i]:,.1f} × {ebit_margin:.2%} = ${ebits[i]:,.1f}"
            for i in range(len(years))
        ],
        "FCF Calculation": [
            f"${revenues[i]:,.1f} × {ebit_margin:.2%} × (1 − {tax_rate:.0%}) "
            f"× (1 − {reinvest_rate:.0%}) = ${fcfs[i]:,.1f}"
            for i in range(len(years))
        ],
        "PV Calculation": [
            f"${fcfs[i]:,.1f} ÷ (1 + {wacc:.4f})^{years[i]} = ${pv_fcfs[i]:,.1f}"
            for i in range(len(years))
        ],
    })
    st.dataframe(
        ...
        column_config={
            "Year": st.column_config.TextColumn(
                "Year",
                help="Projection year (1 through N)",
                width="small",
            ),
            "Revenue ($M)": st.column_config.TextColumn(
                "Revenue ($M)",
                help="Prior year revenue × (1 + growth rate). Year 1 = Base × (1+g).",
            ),
            "EBIT ($M)": st.column_config.TextColumn(
                "EBIT ($M)",
                help="Revenue × EBIT margin",
            ),
            "NOPAT ($M)": st.column_config.TextColumn(
                "NOPAT ($M)",
                help="Net Operating Profit After Tax = EBIT × (1 − tax rate)",
            ),
            "Reinvestment ($M)": st.column_config.TextColumn(
                "Reinvestment ($M)",
                help="NOPAT × Reinvestment rate. Capex + change in working capital.",
            ),
            "FCF ($M)": st.column_config.TextColumn(
                "FCF ($M)",
                help="Free Cash Flow to the Firm = NOPAT − Reinvestment",
            ),
            "Discount Factor": st.column_config.TextColumn(
                "Discount Factor",
                help="1 ÷ (1 + WACC)^t. Brings future cash flows back to today's value.",
            ),
            "PV of FCF ($M)": st.column_config.TextColumn(
                "PV of FCF ($M)",
                help="FCF × Discount Factor. The amount the future FCF is worth today.",
            ),
            "EBIT Calculation": st.column_config.TextColumn(
                "EBIT Calculation",
                help="The EBIT formula with this year's numbers plugged in: Revenue × EBIT margin.",
                width="large",
            ),
            "FCF Calculation": st.column_config.TextColumn(
                "FCF Calculation",
                help="The full FCF formula with this year's numbers plugged in. Use this to verify the math.",
                width="large",
            ),
            "PV Calculation": st.column_config.TextColumn(
                "PV Calculation",
                help="The discounting formula with this year's FCF and period number plugged in.",
                width="large",
            ),
        },
    )
...
with tab_results:
    ...
    # Build a numeric (unformatted) version of the projection for clean Excel import
    csv_df = pd.DataFrame({
        "Year": years,
        "Revenue ($M)": [round(x, 2) for x in revenues],
        "EBIT ($M)": [round(x, 2) for x in ebits],
        "NOPAT ($M)": [round(x, 2) for x in nopats],
        "Reinvestment ($M)": [round(x, 2) for x in reinvestments],
        "FCF ($M)": [round(x, 2) for x in fcfs],
        "Discount Factor": [round(x, 6) for x in discount_factors],
        "PV of FCF ($M)": [round(x, 2) for x in pv_fcfs],
    })
